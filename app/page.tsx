...
bility */}
      <div className="absolute inset-0 bg-[radial-gradient(70%_60%_at_50%_40%,transparent_0%,transparent_55%,rgba(0,0,0,0.45)_100%)]" />
    </div>
    ...
    <div className="max-w-6xl mx-auto">
      <div className="text-center">
        ...
        <div className="mt-4 inline-flex items-center px-4 py-2 rounded-xl bg-white/10 border border-white/15 text-white/90 backdrop-blur">
      ...
      <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
        {[
          { title: "Diagnóstico personalizado", icon: "Gauge" },
          { title: "Mapeamento de processos críticos", icon: "Workflow" },
          { title: "Criação dos agentes IA sob medida", icon: "Brain" },
          { title: "Integração com seu time e sistemas", icon: "Link2" },
          { title: "Monitoramento e evolução contínua", icon: "LineChart" },
        ].map((step, idx) => (
          <div key={idx} className="flex flex-col items-center text-center">
            <div className="w-16 h-16 rounded-xl bg-white/5 border border-white/15 flex items-center justify-center text-white">
              {step.icon === "Gauge" && <Gauge className="h-9 w-9" />}
              {step.icon === "Workflow" && <Workflow className="h-9 w-9" />}
              {step.icon === "Brain" && <Brain className="h-9 w-9" />}
              {step.icon === "Link2" && <Link2 className="h-9 w-9" />}
              {step.icon === "LineChart" && <LineChart className="h-9 w-9" />}
            </div>
            <p className="mt-4 text-sm sm:text-base leading-snug text-white/90">
              {step.title}
            </p>
          </div>
        ))}
      ...
      <div className="relative mt-10 hidden md:block">
        <div className="h-[2px] w-full rounded bg-white/85" />
        <div className="absolute inset-0 grid grid-cols-5">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="relative">
              <div className="absolute left-1/2 top-1/2 h-5 w-[2px] -translate-x-1/2 -translate-y-1/2 bg-white/90" />
            </div>
          ))}
        </div>
      </div>
...
<div className="max-w-6xl mx-auto">
<h2 className="text-3xl font-bold t03c78]">{card.title}</h3>
